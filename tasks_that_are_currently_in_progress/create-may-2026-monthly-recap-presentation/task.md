...
## Goals
- Calculate exact/estimated savings for migrating away from Recall AI & AWS to Splain (which yields zero custom system maintenance).
...
## Plan
1. [x] Research spending details for Recall AI, AWS, Railway, Zoom, and Splain from CSVs or documentation.
   - Evidence: Found Recall AI ($367.00/mo), Railway ($10.00/mo), and Zoom (12 seats Pro) in the master subscription CSV.
2. [x] Calculate the cost savings if we switch to the Splain plan + sunset custom n8n/Autobot.
   - Evidence: Estimated monthly savings of ~$293 to ~$307/mo ($3.5K to $3.6K/yr) by replacing Recall AI ($367) + Railway ($10) with Splain ($70 - $84 for 10-12 Zoom users) and shutting down the AWS/Railway infrastructure completely.
...
5. [x] Commit and push changes to the `mark-dev` branch.
   - Evidence: Staged changes, checked diff for security leaks, committed, and pushed to remote `mark-dev` branch successfully (Commit hash: `148637b`).
